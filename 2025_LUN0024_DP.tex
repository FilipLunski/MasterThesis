...
\CzechAbstract{ Rozvoj technologie v posledních letech, zejména v oblasti umělé inteligence a neuronových sítí, umožňuje komplexnější analýzy chování osob. Dalo by se tak bezpečnostní kamery využit pro zajištění bezpečnosti osob. V této práci je tak navrženo řešení pro detekci pádu osob v obrazovém toku v reálném čase. Řešení je postaveno na kombinaci dvou neuronových sítí. První detekuje všechny osoby v obraze a jejich klíčové body, druhá tyto body klasifikuje do tříd \textit{normální} a \textit{upadl}. V práci je popsán výběr detektoru pózy, návrh architektury klasifikační sítě a implementace výsledného detektoru pádu. Ve výsledném řešení je použit model YOLO11-pose a rekurenetní neuronová síť postavená na architektuře GRU. }
...
% \part{Teorie}
\input{Chapters/101_NN}
\input{Chapters/102_CNN}
\input{Chapters/110_RNN}
% \part{Detekce osob a jejich pozice}
\input{Chapters/200_Goal}
\input{Chapters/201_PoseDetection}

\input{Chapters/220_ClassificationImplementation}
% \input{Chapters/230_ClassificationExperiments}

\input{Chapters/240_FallDetectionImplementation}
% \part{Detekce pádu}
\input{Chapters/901_Conclusion}
