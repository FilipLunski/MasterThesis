...
Tento algoritmus je poměrně robustní s pohledu světelných podmínek a rozlišení
...
schovány za jinou části těla), totiž často nedetekuje člověka vůbec, nejčastěji
...
přesných výsledku, asi nejlepších ze všech testovaných algoritmů. Na rozdíl od
jiných algoritmů totiž, pokud detekoval osobu, vždy velmi přesně označil
všechny její klíčové body, v rámci možnosti i ty, které byly hůře viditelné
(např. schovány za jinou části těla). Na obrázku \ref{fig:ym_comparison} je
vidět rozdíl v přesnosti detekce klíčových bodů mezi nejmenší variantou
BlazePose a druhou největší variantou YOLO, kdy BlazePose dosahuje mnohem vetší
...
by tak mohla být v našem řešení použitelná, což by nám taky dávalo možnost
...
rozlišením obrazu. Ve většině případu sice detekuje klíčové body velmi přesně,
...
od osob, pravděpodobně se pro naší aplikaci nebude hodit.
...
\ref{fig:torchvision_bad}. Zároveň oproti BlazePose nemá takový problém z
...
\begin{figure}
    ...
    \caption{Porovnání přesnosti variant modelu YOLO. Zleva: $Nano$, $Small$, $Medium$, $Large$, $Xlarge$}
...
je stoje či chůze. Stejně i u menších variant YOLO přesnost prudce klesá v méně
typických pózách či natočeních v obraze. Jelikož ale je naše práce postavena
právě na detekování více netypické postavy, je pro nás důležité detekovat
pozici ve všech situacích.
